...
struct ServiceAddressTest : public ::testing::Test
{
  ...
  const std::string arqma =
      "4rqhc5x5do3uh3usttnqz9ek3uuoemdrwzto1hciwim9f947or6y.arqma";
  const std::string sub = "arqnet.test";
  const std::string invalid =
      "4rqhc5x5do3uh3usttnqz9ek3uuoemdrwzto1hciwim9f947or6y.net";
};
...
{
  llarp::service::Address addr;
  const std::string bad = arqma + ".net";
...
{
  llarp::service::Address addr;
  const std::string bad = sub + "." + arqma + ".net";
  ASSERT_FALSE(addr.FromString(bad, ".net"));
}

TEST_F(ServiceAddressTest, TestParseSNodeNotArqma)
{
  llarp::service::Address addr;
  ASSERT_TRUE(addr.FromString(snode, ".snode"));
  ASSERT_FALSE(addr.FromString(snode, ".arqma"));
}

TEST_F(ServiceAddressTest, TestParseArqmaNotSNode)
{
  llarp::service::Address addr;
  ASSERT_FALSE(addr.FromString(arqma, ".snode"));
  ASSERT_TRUE(addr.FromString(arqma, ".arqma"));
}

TEST_F(ServiceAddressTest, TestParseArqmaWithSubdomain)
{
  llarp::service::Address addr;
  const std::string addr_str = sub + "." + arqma;
  ASSERT_TRUE(addr.FromString(addr_str, ".arqma"));
