#pragma once

#include <cstdint>
#include <array>

namespace llarp
{
  // Example: arqnet-1.2.3-abc
  extern const std::array<uint16_t, 3> VERSION;         // [1, 2, 3]
  extern const std::array<uint64_t, 4> ROUTER_VERSION;  // [proto, 1, 2, 3]
  extern const char* const VERSION_STR;                 // "1.2.3"
  extern const char* const VERSION_TAG;                 // "abc"
  extern const char* const VERSION_FULL;                // "arqnet-1.2.3-abc"

  extern const char* const RELEASE_MOTTO;               // "Privacy is a Must"
  extern const char* const DEFAULT_NETID;               // e.g. "arqmainnet"
}  // namespace llarp
