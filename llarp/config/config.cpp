...
{
  const char *
  arqnetEnv(string_view suffix)
  {
    std::string env;
    env.reserve(7 + suffix.size());
    env.append("ARQNET_"s);
  ...
  {
    if(const char *ptr = arqnetEnv(envNameSuffix))
      return ptr;
  ...
  void
  ArqmadConfig::fromSection(string_view key, string_view val)
  {
    ...
    {
      arqmadRPCAddr = str(val);
    }
    if(key == "username")
    {
      arqmadRPCUser = str(val);
    }
    if(key == "password")
    {
      arqmadRPCPassword = str(val);
    }
  }
  ...
  Config::parse(const ConfigParser &parser)
  {
    ...
    arqmad    = find_section< ArqmadConfig >(parser, "arqmad");
...
#endif
    return homedir / fs::path(".arqnet");
  }

  fs::path
  GetDefaultConfigPath()
  {
    return GetDefaultConfigDir() / "arqnet.ini";
  }
                            ...
                            bool isRouter)
{
  ...
  f << std::endl;
  f << "# uncomment following line to set router nickname to 'arqnet'"
    << std::endl;
  f << "#nickname=arqnet\n";
  ...
  f << "[system]\n";
  f << "user=" << DEFAULT_ARQNET_USER << std::endl;
  f << "group=" << DEFAULT_ARQNET_GROUP << std::endl;
  f << "pidfile=" << basepath << "arqnet.pid\n";
  f << "\n\n";
...
{
  f << "# arqmad settings (disabled by default)\n";
  f << "[arqmad]\n";
  f << "enabled=false\n";
  f << "jsonrpc=127.0.0.1:19994\n";
...
llarp_ensure_client_config(std::ofstream &f, std::string basepath)
{
  ...
  const std::string snappExample_fpath = basepath + "snapp-example.ini";
  {
    ...
    if(example_f.is_open())
    {
      ...
      {
        llarp::LogError(
            "Couldn't easily detect a private range to map arqnet onto");
        return false;
      }
     */
      ...
      example_f << "# keyfile is the path to the private key of the snapp, "
                   "your .arqma is tied to this key, DON'T LOSE IT\n";
