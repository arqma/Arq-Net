...
namespace llarp
{
  ...
  FindFreeTun()
  {
    ...
    if(num == 255)
    {
      llarp::LogError("Could not find any free arqmatun interface names");
      return "";
    }
// include arqmatun prefix to communicate result is valid
#if defined(ANDROID) || defined(RPI)
    char buff[IFNAMSIZ + 1] = {0};
    snprintf(buff, sizeof(buff), "arqmatun%u", num);
    return buff;
#else
    return "arqmatun" + std::to_string(num);
#endif
  }
