...
def exit_sessions_main():
    if len(sys.argv) == 2 and sys.argv[1] == 'config':
        print("graph_title arqnet exit sessions")
        print("graph_vlabel sessions")
        print("graph_category network")
        print("graph_info This graph shows the number of exit sessions on a arqnet exit")
...
def peers_main():
    if len(sys.argv) == 2 and sys.argv[1] == 'config':
        print("graph_title arqnet peers")
        print("graph_vlabel peers")
        print("graph_category network")
        print("graph_info This graph shows the number of node to node sessions of this arqnet router")
        print("_peers_outbound.info Number of outbound arqnet peers")
        print("_peers_inbound.info Number of inbound arqnet peers")
        print("_peers_outbound.label outbound peers")
        print("_peers_inbound.label inbound peers")
        print("_peers_clients.info Number of arqnet client peers")
        print("_peers_clients.label arqnet client peers")
    else:
        inbound = Dict(int)
        outbound = Dict(int)
        clients = Dict(int)
...
if __name__ == '__main__':
    ...
    if exe == 'arqnet_peers':
        peers_main()
    elif exe == 'arqnet_exit':
        exit_sessions_main()
    else:
        print(
            'please symlink this as `arqnet_peers` or `arqnet_exit` in munin plugins dir')
