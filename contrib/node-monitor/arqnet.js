...
const VERSION = 1.0
console.log('arqnet launcher version', VERSION, 'registered')
...
function testDNSForArqnet(server, cb) {
  const resolver = new dns.Resolver()
  resolver.setServers([server])
  // incase server is 127.0.0.1:undefined
  try {
    resolver.resolve('localhost.arqma', function (err, records) {
      if (err) console.error('NETWORK: localhost.arqma resolve err:', err)
      //log(server, 'localhost.arqma test results', records)
      cb(records)
    })
  } catch (e) {
    console.error('NETWORK: testDNSForArqnet error, incorrect server?', server)
    cb()
  }
}
...
function lookup(host, cb) {
  ...
  //console.log('arqnet lookup servers', runningConfig.dns.bind)
    ...
    if (err) {
      ...
      if (err.code == 'ENOTFOUND') {
        ...
        if (err.code == 'ETIMEOUT') {
          ...
          console.error('arqnet lookup unknown err', err)
        }
    }
    //console.log(host, 'arqnet dns test results', records)
    cb(records)
  })
}

function findArqNetDNS(cb) {
  ...
  function checkDone() {
    if (shuttingDown) {
      //if (cb) cb()
      log('not going to start arqnet, shutting down')
      return
    }
    checksLeft--
    if (checksLeft <= 0) {
  ...
  for(var i in resolvers) {
    ...
    if (idx != -1) {
      ...
      checksLeft++
      testDNSForArqnet(server, function(isArqnet) {
        if (isArqnet) {
          // arqnet
          console.log(server, 'is a arqnet DNS server')
          servers.push(server)
        }
        checkDone()
      })
  ...
  */
  ...
  for (var i in localIPs) {
    ...
    checksLeft++
    testDNSForArqnet(server, function (isArqnet) {
      if (isArqnet !== undefined) {
        // arqnet
        log(server, 'is a arqnet DNS server')
        servers.push(server)
      }
      checkDone()
    })
  ...
  for (var i in resolvers) {
    ...
    if (idx != -1) {
      ...
      testDNSForArqnet(server, function (isArqnet) {
        if (isArqnet === undefined) {
          // not arqnet
          log(server, 'is not a arqnet DNS server')
          servers.push(server)
        }
        checkDone()
      })
      ...
      if (idx != -1) {
        ...
        resolver.resolve('localhost.arqma', function(err, records) {
          ...
          if (records === undefined) {
            // not arqnet
            console.log(server, 'is not a arqnet DNS server')
            servers.push(server)
          }
          checkDone()
        })
...
// this can really delay the start of arqnet
...
function generateINI(config, need, markDone, cb) {
  ...
  //const data = fs.readFileSync(homeDir + '/.arqnet/arqnet.ini', 'utf-8')
  //const jConfig = iniToJSON(data)
  //console.dir(jConfig)
  //const iConfig = jsonToINI(jConfig)
  //console.log(iConfig)
  var upstreams, arqnet_free53Ip, arqnet_nic
  var use_arqnet_rpc_port = config.rpc_port
  var arqnet_bootstrap_path = homeDir + '/.arqnet/bootstrap.signed'
  var arqnet_nodedb = homeDir + '/.arqnet/netdb'
  if (config.netid) {
    arqnet_nodedb += '-' + config.netid
  }
  if (!fs.existsSync(arqnet_nodedb)) {
    log('making', arqnet_nodedb)
    mkDirByPathSync(arqnet_nodedb)
  }
  var upstreamDNS_servers = []
  var params = {
    upstreamDNS_servers: upstreamDNS_servers,
    arqnet_free53Ip: arqnet_free53Ip,
    arqnet_nodedb: arqnet_nodedb,
    arqnet_bootstrap_path: arqnet_bootstrap_path,
    arqnet_nic: arqnet_nic,
    use_arqnet_rpc_port: use_arqnet_rpc_port,
  }
  if (config.bootstrap_url) {
    httpGet(config.bootstrap_url, function (bootstrapData) {
      if (bootstrapData) {
        cleanUpBootstrap = true
        const tmpRcPath = os.tmpdir() + '/' + randomString(8) + '.arqnet_signed'
        fs.writeFileSync(tmpRcPath, bootstrapData, 'binary')
        log('boostrap wrote', bootstrapData.length, 'bytes to', tmpRcPath)
        //arqnet_bootstrap_path = tmpRcPath
        params.arqnet_bootstrap_path = tmpRcPath
  ...
  httpGet('http://' + config.rpc_ip + ':' + config.rpc_port, function (testData) {
    ...
    if (testData !== undefined) {
      ...
      use_arqnet_rpc_port = use_arqnet_rpc_port + 1
      params.use_arqnet_rpc_port = use_arqnet_rpc_port
    }
  ...
  getNetworkIP(function (e, ip) {
    ...
    arqnet_nic = getIfNameFromIP(ip)
    params.arqnet_nic = arqnet_nic
    params.interfaceIP = ip
    log('detected outgoing interface', arqnet_nic)
    ...
    findFreePort53(tryIps, 0, function (free53Ip) {
      if (free53Ip === undefined) {
        console.error('NETWORK: Cant automatically find an IP to put a arqnet DNS server on')
        // can't handle the exits here because we don't know if it's an actual requirements
        if (done.dnsBind !== undefined) {
          process.exit()
        }
      }
      arqnet_free53Ip = free53Ip
      params.arqnet_free53Ip = free53Ip
...
function applyConfig(file_config, config_obj) {
  ...
  if (file_config.arqmad && file_config.arqmad.network == "test") {
...
function generateSerivceNodeINI(config, cb) {
  ...
  function markDone(completeProcess, params) {
    ...
    var keyPath = homeDir + '/.arqma/'
    //
    if (config.arqmad.data_dir) {
      keyPath = config.arqmad.data_dir
      // make sure it has a trailing slash
      if (keyPath[keyPath.length - 1] != '/') {
        keyPath += '/'
      }
    }
    if (config.arqmad.network == "test" || config.arqmad.network == "demo") {
      keyPath += 'testnet/'
    }
    keyPath += 'key'
    ...
    log('Drafting arqnet service node config')
    ...
    runningConfig = {
      ...
      dns: {
        ...
        bind: params.arqnet_free53Ip + ':53',
      },
      netdb: {
        dir: params.arqnet_nodedb,
      },
      bind: {
        // will be set after
      },
      ...
      api: {
        enabled: true,
        bind: config.rpc_ip + ':' + params.use_arqnet_rpc_port
      },
      arqmad: {
        enabled: true,
        jsonrpc: config.arqmad.rpc_ip + ':' + config.arqmad.rpc_port,
        username: config.arqmad.rpc_user,
        password: config.arqmad.rpc_pass,
    ...
    runningConfig.bind[params.arqnet_nic] = config.public_port
    if (config.internal_port) {
      runningConfig.bind[params.arqnet_nic] = config.internal_port
    }
...
function generateClientINI(config, cb) {
  ...
  function markDone(completeProcess, params) {
    ...
    if (!params.use_arqnet_rpc_port) {
      // use default because we enable it
      params.use_arqnet_rpc_port = 1190
    }
    log('Drafting arqnet client config')
    runningConfig = {
      router: {
        nickname: 'ldl',
      },
...
var shuttingDown
var arqnet
var arqnetLogging = true

function preLaunchArqnet(config, cb) {
  ...
  if (os.platform() == 'linux') {
    ...
    exec('getcap ' + config.binary_path, function (error, stdout, stderr) {
      ...
      // src/arqma/arqnet = cap_net_bind_service,cap_net_admin+eip
  ...
  // arqnet will crash if this file is zero bytes
  ...
  const tmpPath = tmpDir + '/' + randomString(8) + '.arqnet_ini'
  cleanUpIni = true
  config.ini_writer(config, function (iniData) {
    if (shuttingDown) {
      //if (cb) cb()
      log('not going to write arqnet config, shutting down')
      return
    }
...
function launchArqnet(config, cb) {
  if (shuttingDown) {
    //if (cb) cb()
    log('not going to start arqnet, shutting down')
    return
  ...
  if (!fs.existsSync(config.ini_file)) {
    log('arqnet config file', config.ini_file, 'does not exist')
    process.exit()
  }
  ...
  arqnet = spawn(config.binary_path, cli_options)

  if (!arqnet) {
    console.error('failed to start arqnet, exiting...')
    // proper shutdown?
    process.exit()
  }
  arqnet.killed = false
  arqnet.stdout.on('data', (data) => {
  ...
  arqnet.stderr.on('data', (data) => {
    if (module.exports.onError) {
      module.exports.onError(data)
    }
  })

  arqnet.on('close', (code) => {
    log(`arqnet process exited with code ${code}`)
    ...
    if (!shuttingDown) {
      if (config.restart) {
        ...
        setTimeout(function () {
          log('arqma_daemon is still running, restarting arqnet')
          launchArqnet(config)
        }, 30 * 1000)
      } else {
        // don't restart...
...
function checkConfig(config) {
  ...
  if (config.binary_path === undefined) config.binary_path = '/usr/local/bin/arqnet'
...
function startServiceNode(config, cb) {
  ...
  // only us arqnet devs will need to make our own seed node
  preLaunchArqnet(config, function () {
    // test arqmad rpc port first
    // also this makes sure the service key file exists
    var url = 'http://' + config.arqmad.rpc_user + ':' + config.arqmad.rpc_pass + '@' + config.arqmad.rpc_ip + ':' + config.arqmad.rpc_port
    log('arqnet waiting for arqmad RPC server')
    waitForUrl(url, function () {
      launchArqnet(config, cb)
    })
  })
}
...
function startClient(config, cb) {
  checkConfig(config)
  if (config.bootstrap_path === undefined && config.bootstrap_url === undefined) config.bootstrap_url = 'https://seed.arqma.com/bootstrap.signed'
...
// intent to stop arqnet and don't restart it
var retries = 0

function stop() {
  shuttingDown = true
  if (arqnet && arqnet.killed) {
    console.warn('arqnet already stopped')
    retries++
    if (retries > 3) {
  ...
  log('requesting arqnet be shutdown')
  if (arqnet && !arqnet.killed) {
    log('sending SIGINT to arqnet', arqnet.pid)
    process.kill(arqnet.pid, 'SIGINT')
    arqnet.killed = true
    // HACK: arqnet on macos can not be killed if rpc port is in use
    var monitorTimerStart = Date.now()
    var monitorTimer = setInterval(function () {
      if (!isPidRunning(arqnet.pid)) {
        // launcher can't exit until this interval is cleared
        clearInterval(monitorTimer)
      } else {
        var diff = Date.now() - monitorTimerStart
        if (diff > 15 * 1000) {
          // reach 15 secs and arqnet is still running
          // escalate it
          console.error('Arqnet is still running 15s after we intentionally stopped arqnet?')
          process.kill(arqnet.pid, 'SIGKILL')
        } else
        if (diff > 30 * 1000) {
          // reach 30 secs and arqnet is still running
          // escalate it
          console.error('Arqnet is still running 30s after we intentionally killed arqnet?')
    ...
    setTimeout(function() {
      try {
        ...
        process.kill(arqnet.pid, 0)
        log('sending SIGKILL to arqnet')
        process.kill(arqnet.pid, 'SIGKILL')
      } catch(e) {
        console.error('Launcher is still running 15s after we intentionally stopped arqnet?')
        var handles = process._getActiveHandles()
        console.log('handles', handles.length)
        for(var i in handles) {
          var handle = handles[i]
...
function getPID() {
  return (arqnet && !arqnet.killed && arqnet.pid) ? arqnet.pid : 0
}
...
function disableLogging() {
  console.log('Disabling arqnet logging')
  arqnetLogging = false
}

function getArqNetIP(cb) {
  function checkDNS() {
    log('arqnet seems to be running, querying', runningConfig.dns.bind)
    // where's our DNS server?
    //log('RunningConfig says our arqnet\'s DNS is on', runningConfig.dns.bind)
    testDNSForArqnet(runningConfig.dns.bind, function (ips) {
      //log('arqnet test', ips)
      if (ips && ips.length) {
        cb(ips[0])
      } else {
        // , retrying
        console.error('cant communicate with arqnet DNS')
        /*
        //process.exit()
        setTimeout(function() {
          getArqNetIP(cb)
  ...
  if (runningConfig.api.enabled) {
    log('wait for arqnet startup', runningConfig.api)
...
module.exports = {
  ...
  findArqNetDNS: findArqNetDNS,
  lookup: lookup,
  isRunning: isRunning,
  stop: stop,
  getArqNetIP: getArqNetIP,
  ...
  onMessage: function (data) {
    if (arqnetLogging) {
      console.log(`arqnet: ${data}`)
    }
  },
  onError: function (data) {
    console.log(`arqneterr: ${data}`)
  },
}
