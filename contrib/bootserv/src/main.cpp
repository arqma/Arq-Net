...
main(int argc, char* argv[])
{
  ...
  const char* confFile = arqnet::bootserv::Config::DefaultPath;
  arqnet::bootserv::Config config;

  arqnet::bootserv::Handler_ptr handler;
  ...
  if(RunCron)
    handler = arqnet::bootserv::NewCronHandler(std::cout);
  else
    handler = arqnet::bootserv::NewCGIHandler(std::cout);
