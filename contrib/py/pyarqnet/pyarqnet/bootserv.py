#!/usr/bin/env python3
#
# python wsgi application for managing many arqnet instances
#

__doc__ = """arqnet bootserv wsgi app
also handles webhooks for CI
run me with via gunicorn pyarqnet.bootserv:app
"""
...
def handle_serve_arqnet(modified_since, respond):
    l = BinHolder('arqnet.zip')
    return l.serve(modified_since, respond)


def fetch_arqnet(j, ref="staging", name="build:linux"):
    holder = BinHolder("arqnet.zip")
    if 'builds' not in j:
        return False
    selected = None
    attrs = dict()
    ...
    if selected and 'id' in selected:
        url = 'Need Updating'.format(selected['id'])
        r = requests.get(url)
...
def app(environ, start_response):
    ...
    if method.upper() == "PUT" and request_body_size > 0:
        ...
        elif environ.get("PATH_INFO") == "/arqnet.zip":
            return handle_serve_arqnet(environ.get("HTTP_IF_MODIFIED_SINCE"),start_response)
        elif environ.get("PATH_INFO") == "/":
            return response("200 OK", "arqnet bootserv", start_response)
        else:
